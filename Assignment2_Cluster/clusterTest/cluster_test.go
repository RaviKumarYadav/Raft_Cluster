...
/*************************************************
	Testing only for BROADCAST messaging
**************************************************/
...
/***********************************************************************************
	Testingo only for "BROADCAST + 60000BYTES_MESSAGE" messaging
***********************************************************************************/
func TestBroadcast_HugeMsgCount(t *testing.T) {
	...
	message := strings.Repeat("Hello John", 6000)

	mutex1.Lock()
	received_count = 0
	mutex1.Unlock()

	fmt.Printf("\n\nTesting BROADCAST + 60000BYTES_MESSAGE ...\n")

	for id := 0; id <= 3; id++ {
		for i := 1; i <= 10; i++ {
			serverArray[id].Outbox() <- &cluster.Envelope{Pid: cluster.BROADCAST, Msg: message}
			send_count += 3
		}
	}
	...
	fmt.Printf("\nTesing BROADCAST + 60000BYTES_MESSAGE result")
	if send_count != received_count {
		t.Errorf("send_count - %v , received_count - %v", send_count, received_count)
	}
}
...
/***********************************************************************
	It takes out all the messages from the Inbox() channel ,
	use it for counting the messages received till now.
***********************************************************************/
